// =====================================================================
// START: 终极健壮版 suanlema-proxy.mjs (请完整替换)
// =====================================================================

// netlify/functions/suanlema-proxy.mjs
console.log("SuanLeMa Proxy Function Loaded (v-Final - Robust Manual Pumping)");

const API_KEY = process.env.SUANLEMA_API_KEY_SECRET;
...
export default async function handler(request) {
  ...
  try {
    ...
    if (!apiResponse.ok) {
      ...
      console.error(`[SuanLeMa Proxy] API Error: ${apiResponse.status}`, errorBody);
    ...
    if (isStream && apiResponse.body) {
        ...
        const pump = async () => {
            ...
            } catch (e) { console.error("[SuanLeMa Proxy] Stream Pumping Error:", e); writer.abort(e); }
        };
        pump();
    ...
    console.error('[SuanLeMa Proxy] Network/internal error', err);
